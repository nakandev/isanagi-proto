...
std::unique_ptr<TargetInfo> AllocateTarget(const llvm::Triple &Triple,
  ...
  case llvm::Triple::{{ xpu }}32le:
    return std::make_unique<{{ Xpu }}32leTargetInfo>(Triple, Opts);
  case llvm::Triple::{{ xpu }}32be:
    return std::make_unique<{{ Xpu }}32beTargetInfo>(Triple, Opts);
  case llvm::Triple::{{ xpu }}64le:
    return std::make_unique<{{ Xpu }}64leTargetInfo>(Triple, Opts);
  case llvm::Triple::{{ xpu }}64be:
    return std::make_unique<{{ Xpu }}64beTargetInfo>(Triple, Opts);
  }
}
} // namespace targets
} // namespace clang
